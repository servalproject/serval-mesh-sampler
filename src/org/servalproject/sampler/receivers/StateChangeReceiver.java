...
/**
 * a broadcast receiver which can receive notifications about the 
 * changes in state of the Serval Mesh software, for example when it is 
 * starting or when it is stoping
 */
public class StateChangeReceiver extends BroadcastReceiver {
	...
	private final String sTag = "StateChangeReceiver";
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals(context.getString(R.string.system_serval_mesh_state_change)) == false) {
		...
		if(intent.getIntExtra("state", -1) != -1) {
			...
			Log.d(sTag, "New State: " + context.getString(mState.getResourceId()));
			
			// send the string back to our activity
			Intent mIntent = new Intent(context, org.servalproject.sampler.StateChangedActivity.class);
			mIntent.putExtra(context.getString(R.string.state_changed_intent_extra), context.getString(mState.getResourceId()));
